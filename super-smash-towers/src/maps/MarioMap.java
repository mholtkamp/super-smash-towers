...
public class MarioMap extends Map
{
	...
	public MarioMap(float difficulty,AssetManager manager)
	{
		
		bg_tex = manager.get("data/maps/mario_map_6.png");
...
//		System.out.println(this.width + " " + this.height);
		...
		song = manager.get("sounds/mario.mp3");
		...
		for (int wave = 0; wave < WAVE_COUNT; wave++)
		{
			...
			{// for first 3 waves, spawn BasicEnemy
				for (int j = 0; j < wave*5 + 5; j++)
					q.add(new Goomba(waypoints,difficulty,manager));
			}
			else if (wave / 3 < 2)
			{// for next 3 waves, spawn Goomba
				for (int j = 0; j < wave*5 + 5; j++)
					q.add(new ShyGuy(waypoints,difficulty,manager));
			}
			else if (wave / 3 < 3)
			...
			{// boss wave
				...
				for (int j = 0; j < wave*5 + 5; j++)
				{
					...
					{
						case 0: q.add(new Goomba(waypoints,difficulty,manager)); break;
						case 1: q.add(new ShyGuy(waypoints,difficulty,manager)); break;
						case 2: q.add(new Koopa(waypoints,difficulty,manager)); break;
						default: q.add(new Goomba(waypoints,difficulty,manager)); break;
					}
