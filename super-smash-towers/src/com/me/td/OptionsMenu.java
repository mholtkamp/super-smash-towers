...
//	private Rectangle Tower4Button;
	...
	public OptionsMenu(OrthographicCamera camera, int level,AssetManager manager)
	{
		this.camera = camera;
		
		OptionBG = manager.get("data/bgoverlay.png");
		if(level == mario)
		{
			...
			OptionM = manager.get("data/pausemenu.png");
		}
		if(level == pokemon || level == zelda || level == galaga)
		{
			...
			OptionM = manager.get("data/pokeMenu.png");
			
		}
		
		texMute = manager.get("data/speaker_off.png");
		texUnmute = manager.get("data/speaker_on.png");
